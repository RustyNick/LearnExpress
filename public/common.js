...
async function cardContainer() {
    ...
    title.id = "title"
    title.innerText = dndRaceInfo.name

    let ageInfo = document.createElement("p")
    ageInfo.id = "ageInfo"
    ageInfo.innerText = "Age: " + dndRaceInfo.age

    let speedInfo = document.createElement("p")
    speedInfo.id = "speedInfo"
    speedInfo.innerText = "Speed: " + dndRaceInfo.speed

    let className = document.createElement("p")
    className.id = "className"
    className.innerText = "class: " + dndClassInfo.name

    let hitDie = document.createElement("p")
    hitDie.id = "hitDie"
    hitDie.innerText = "Hit Die: D" + dndClassInfo.hit_die

    cardBody.append(title, ageInfo, speedInfo, className, hitDie);
